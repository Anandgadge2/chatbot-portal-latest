...
async function updateFlow() {
  ...
  const existingStepIds = new Set(flow.steps.map(s => s.stepId));
  ...
  const updatedSteps = flow.steps.map(step => {
    if (step.stepId === 'grv_confirm_en') {
      step.buttons = step.buttons.map(b => b.id === 'cancel_grv_en' ? { ...b, nextStepId: 'grv_cancel_msg_en' } : b);
      step.expectedResponses = step.expectedResponses.map(r => r.value === 'cancel_grv_en' ? { ...r, nextStepId: 'grv_cancel_msg_en' } : r);
    }
    if (step.stepId === 'grv_confirm_hi') {
      step.buttons = step.buttons.map(b => b.id === 'cancel_grv_hi' ? { ...b, nextStepId: 'grv_cancel_msg_hi' } : b);
      step.expectedResponses = step.expectedResponses.map(r => r.value === 'cancel_grv_hi' ? { ...r, nextStepId: 'grv_cancel_msg_hi' } : r);
    }
    if (step.stepId === 'grv_confirm_or') {
      step.buttons = step.buttons.map(b => b.id === 'cancel_grv_or' ? { ...b, nextStepId: 'grv_cancel_msg_or' } : b);
      step.expectedResponses = step.expectedResponses.map(r => r.value === 'cancel_grv_or' ? { ...r, nextStepId: 'grv_cancel_msg_or' } : r);
    }
    if (step.stepId === 'apt_confirm_en') {
      step.buttons = step.buttons.map(b => b.id === 'cancel_apt_en' ? { ...b, nextStepId: 'apt_cancel_msg_en' } : b);
      step.expectedResponses = step.expectedResponses.map(r => r.value === 'cancel_apt_en' ? { ...r, nextStepId: 'apt_cancel_msg_en' } : r);
    }
    if (step.stepId === 'apt_confirm_hi') {
      step.buttons = step.buttons.map(b => b.id === 'cancel_apt_hi' ? { ...b, nextStepId: 'apt_cancel_msg_hi' } : b);
      step.expectedResponses = step.expectedResponses.map(r => r.value === 'cancel_apt_hi' ? { ...r, nextStepId: 'apt_cancel_msg_hi' } : r);
    }
    if (step.stepId === 'apt_confirm_or') {
      step.buttons = step.buttons.map(b => b.id === 'cancel_apt_or' ? { ...b, nextStepId: 'apt_cancel_msg_or' } : b);
      step.expectedResponses = step.expectedResponses.map(r => r.value === 'cancel_apt_or' ? { ...r, nextStepId: 'apt_cancel_msg_or' } : r);
    }
    return step;
  });
