...
export default function CompanyDrillDown() {
    ...
    try {
      ...
      setStats({
        ...
        totalDepartments: deptRes.success
          ? deptRes.data.departments?.length || 0
          : 0,
        totalGrievances: grievancesRes.success
          ? grievancesRes.data.grievances?.length || 0
          : 0,
        totalAppointments: appointmentsRes.success
          ? appointmentsRes.data.appointments?.length || 0
          : 0,
        activeUsers: usersRes.success
          ? usersRes.data.users?.filter((u: User) => u.isActive).length || 0
          : 0,
        pendingGrievances: grievancesRes.success
          ? grievancesRes.data.grievances?.filter(
              (g: Grievance) => g.status === "PENDING",
            ).length || 0
          : 0,
        resolvedGrievances: grievancesRes.success
          ? grievancesRes.data.grievances?.filter(
              (g: Grievance) => g.status === "RESOLVED",
            ).length || 0
          : 0,
      });
    } catch (error: any) {
  ...
  const handleBulkDeleteGrievances = async () => {
    ...
    if (
      !confirm(
        `Are you sure you want to delete ${selectedGrievances.size} grievance(s)?`,
      )
    )
      return;
    setIsDeleting(true);
  ...
  const handleBulkDeleteDepartments = async () => {
    ...
    if (
      !confirm(
        `Are you sure you want to delete ${selectedDepartments.size} department(s)?`,
      )
    )
      return;
    setIsDeleting(true);
