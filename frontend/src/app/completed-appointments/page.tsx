...
export default function CompletedAppointmentsPage() {
  ...
  useEffect(() => {
    if (!authLoading && user && user.role !== "COMPANY_ADMIN") {
      toast.error(
        "Access denied. Completed appointments are only available for Company Admins.",
      );
  ...
  const filteredAppointments = useMemo(() => {
    let filtered = appointments.filter((a) => {
      ...
      if (filters.dateRange !== "all") {
        ...
        if (filters.dateRange === "today" && completedDate < today)
          return false;
        if (filters.dateRange === "week" && completedDate < weekAgo)
          return false;
        if (filters.dateRange === "month" && completedDate < monthAgo)
          return false;
      }
      ...
      if (
        ...
        !a.purpose?.toLowerCase().includes(filters.search.toLowerCase())
      )
        return false;
      return true;
    });
      ...
      <div className="flex h-screen items-center justify-center bg-gradient-to-br from-slate-50 to-purple-50/30">
        <div className="text-center">
          ...
          <p className="text-slate-600 mb-6">
            Completed appointments are only available for Company Admins.
          </p>
          <button
              ...
              <table className="w-full">
                <thead className="bg-slate-50 border-b border-slate-200 whitespace-nowrap">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left">
                      <button className="group flex items-center space-x-1.5 text-[10px] font-black text-slate-500 uppercase tracking-widest hover:text-slate-700 transition-colors">
                        <span>App ID</span>
                        <ArrowUpDown
                          className={`w-3 h-3 transition-colors ${sortConfig.key === "appointmentId" ? "text-slate-700" : "text-slate-300 group-hover:text-slate-400"}`}
                        />
                      </button>
                    </th>
                    <th className="px-6 py-4 text-left">
                      ...
                      >
                        ...
                        <ArrowUpDown
                          className={`w-3 h-3 transition-colors ${sortConfig.key === "citizenName" ? "text-slate-700" : "text-slate-300 group-hover:text-slate-400"}`}
                        />
                      </button>
                    </th>
                    ...
                    <th className="px-6 py-4 text-left">
                      ...
                      >
                        ...
                        <ArrowUpDown
                          className={`w-3 h-3 transition-colors ${sortConfig.key === "appointmentDate" ? "text-slate-700" : "text-slate-300 group-hover:text-slate-400"}`}
                        />
                      </button>
                    </th>
                    <th className="px-6 py-4 text-left">
                      ...
                      >
                        ...
                        <ArrowUpDown
                          className={`w-3 h-3 transition-colors ${sortConfig.key === "completedAt" ? "text-slate-700" : "text-slate-300 group-hover:text-slate-400"}`}
                        />
                      </button>
                    </th>
                ...
                <tbody className="divide-y divide-slate-200">
                  {filteredAppointments.map((appointment, index) => {
                    ...
                    const completedDate = (appointment as any).completedAt
                      ? new Date((appointment as any).completedAt)
                      : new Date(appointment.updatedAt);

                    return (
                      <tr
                        key={appointment._id}
                        className="hover:bg-emerald-50/30 transition-colors border-b border-slate-100"
                      >
