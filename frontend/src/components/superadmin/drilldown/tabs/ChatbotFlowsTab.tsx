    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-slate-200">
         <div className="min-w-0 flex-1">
            <h2 className="text-[10px] sm:text-sm font-bold text-slate-800 uppercase tracking-tight truncate">Response Pipelines</h2>
            <p className="text-[13px] sm:text-[14px] text-slate-500 font-medium font-mono uppercase tracking-widest truncate">{company?.name || "Neural Node"}</p>
         </div>
          <div className="flex items-center gap-1.5 sm:gap-2 self-end sm:self-auto">
            <Button onClick={() => fetchData(true)} variant="ghost" className="h-7 w-7 sm:h-8 sm:w-8 p-0 text-slate-400">
                <RefreshCw className={`w-3 h-3 sm:w-3.5 sm:h-3.5 ${refreshing ? "animate-spin" : ""}`} />
            </Button>
            <Button onClick={handleCreateFlow} className="h-8 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg px-2.5 sm:px-4 text-[12px] sm:text-[14px] font-bold uppercase tracking-wider">
                <Plus className="w-3 h-3 sm:w-3.5 sm:h-3.5 mr-1 sm:mr-1.5" />
                <span className="hidden xs:inline">Build Custom Pipeline</span>
      ...
      {whatsappConfig && whatsappConfig._id && whatsappConfig.isActive ? (
        <Card className="rounded-xl border border-slate-200 shadow-sm overflow-hidden bg-slate-50/30">
           <CardContent className="p-3 sm:p-4 grid grid-cols-1 sm:grid-cols-[auto_1fr_auto_auto] items-center gap-3 sm:gap-8">
              <div className="flex items-center gap-2.5 sm:gap-3">
                 ...
                 <div className="min-w-0">
                    <p className="text-[10px] sm:text-[15px] text-slate-400 font-black uppercase tracking-widest truncate">Active Endpoint</p>
                    <p className="text-[11px] sm:text-xs font-bold text-slate-700 truncate">{whatsappConfig.displayPhoneNumber || whatsappConfig.phoneNumber}</p>
                 </div>
              </div>
              <div className="hidden sm:block w-px h-10 bg-slate-200" />
              <div className="flex items-center gap-2.5 sm:gap-3">
                 <div className="w-8 h-8 sm:w-10 sm:h-10 bg-indigo-100 rounded-lg sm:rounded-xl flex items-center justify-center shrink-0">
                    <Workflow className="w-4 h-4 sm:w-5 sm:h-5 text-indigo-600" />
                 </div>
                 <div className="min-w-0">
                    <p className="text-[10px] sm:text-[15px] text-slate-400 font-black uppercase tracking-widest truncate">Active Logic Matrix</p>
                    <p className="text-[11px] sm:text-xs font-bold text-slate-700 truncate">
                      ...
                      {activeFlow && <span className="ml-1.5 px-1 py-0.5 bg-indigo-50 text-indigo-600 rounded text-[10px] font-black uppercase tracking-tighter">v{activeFlow.version || 1}</span>}
                    </p>
                 </div>
              </div>
              {activeFlow && (
                <Button 
                  ...
                  variant="outline" 
                  className="w-full sm:w-auto h-8 sm:h-9 rounded-lg sm:rounded-xl border-indigo-200 text-indigo-600 hover:bg-indigo-50 font-bold text-[12px] sm:text-[14px] uppercase tracking-wider mt-1 sm:mt-0"
                >
           ...
           <CardContent className="p-4 sm:p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                 ...
                 <div>
                    ...
                    <p className="text-[14px] sm:text-xs text-slate-600 mt-0.5 sm:mt-1">Chatbot flows require an active WhatsApp configuration to deploy.</p>
                 </div>
              </div>
              <Button onClick={() => {}} className="w-full sm:w-auto bg-amber-600 hover:bg-amber-700 h-9 font-bold text-[14px] sm:text-xs uppercase tracking-wider">
                 Initialize WhatsApp
              </Button>
           </CardContent>
        </Card>
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="w-full sm:w-auto overflow-x-auto no-scrollbar -mx-1 px-1">
            <TabsList className="w-max sm:w-auto flex min-w-full">
              <TabsTrigger value="your-flows" className="flex-1 sm:flex-none px-4 sm:px-6 h-8 rounded-lg text-[12px] sm:text-[14px] font-black uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-indigo-600 data-[state=active]:shadow-sm whitespace-nowrap">
                Operational Nodes ({operationalFlows.length})
              </TabsTrigger>
              <TabsTrigger value="templates" className="flex-1 sm:flex-none px-4 sm:px-6 h-8 rounded-lg text-[12px] sm:text-[14px] font-black uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-indigo-600 data-[state=active]:shadow-sm whitespace-nowrap">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {selectedFlows.size > 0 && (
              <Button
                ...
                className="h-7 sm:h-8 text-[11px] sm:text-[15px] font-black uppercase tracking-widest bg-red-600 hover:bg-red-700 animate-in zoom-in duration-200"
              >
            ...
            {operationalFlows.length > 0 && (
              <Button
                ...
                className="h-7 sm:h-8 text-[11px] sm:text-[15px] font-black uppercase tracking-widest border-slate-200 hover:bg-slate-50"
               >
                 {selectedFlows.size === operationalFlows.length ? "Deselect All" : "Select All"}
               </Button>
             )}
          </div>
        </div>
        ...
        <TabsContent value="your-flows" className="space-y-4 pt-2">
           {operationalFlows.length === 0 ? (
             <div className="flex flex-col items-center justify-center py-16 sm:py-20 bg-white border border-slate-200 rounded-2xl border-dashed">
                ...
                <p className="text-[14px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest text-center">No Operational Pipelines Found</p>
                <Button onClick={handleCreateFlow} variant="ghost" className="mt-4 text-[15px] sm:text-[14px] font-black uppercase text-indigo-600 hover:bg-indigo-50">Start Architecture Design</Button>
             </div>
           ) : (
                ...
                {operationalFlows.map((flow) => (
                  <Card key={flow._id} className={`rounded-xl border transition-all hover:shadow-md relative group/card ${flow.isActive ? "border-indigo-200 bg-white shadow-indigo-50" : "border-slate-100 bg-white shadow-sm"}`}>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                             <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-1.5 sm:gap-2 mb-1 flex-wrap">
                                   <h3 className="text-[13px] sm:text-xs font-bold text-slate-800 uppercase tracking-tight truncate max-w-[140px] sm:max-w-none">{flow.flowName || flow.name}</h3>
                                   {flow.isActive && <span className="px-1.5 py-0.5 bg-emerald-50 text-emerald-600 rounded text-[10px] font-black uppercase tracking-widest border border-emerald-100">Deployed</span>}
                                   <span className="px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded text-[10px] font-black uppercase tracking-widest border border-slate-200">v{flow.version || 1}</span>
                                </div>
                                <p className="text-[12px] sm:text-[15px] text-slate-500 line-clamp-1">{flow.flowDescription || flow.description || "Experimental pipeline node"}</p>
                                <div className="flex items-center gap-3 sm:gap-4 mt-2.5 sm:mt-3">
                                   <span className="text-[10px] sm:text-[15px] font-black text-slate-400 uppercase tracking-widest">Type: <span className="text-indigo-600 font-black">{flow.flowType || "Custom"}</span></span>
                                   <span className="text-[10px] sm:text-[15px] font-black text-slate-400 uppercase tracking-widest">Steps: <span className="text-slate-800 font-black">{flow.steps?.length || 0}</span></span>
                                </div>
                             </div>
                     ...
                     <CardContent className="py-3 px-4 sm:px-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/20">
                        <div className="text-[15px] sm:text-[14px] text-slate-500 font-medium overflow-x-auto no-scrollbar whitespace-nowrap pb-1 sm:pb-0">
                           <span className="font-bold text-slate-700 uppercase tracking-widest mr-2">Triggers:</span>
                           {flow.triggers?.map((t: any, i: number) => (
                             <span key={i} className="inline-block px-2 py-0.5 bg-white border border-slate-200 rounded text-[15px] sm:text-[14px] font-bold text-slate-600 mr-1.5">&quot;{t.triggerValue}&quot;</span>
                           )) || <span className="italic">N/A</span>}
                        </div>
                        {flow.isActive ? (
                           <div className="flex items-center justify-center gap-1 px-2.5 py-1.5 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-lg text-[12px] sm:text-[14px] font-black uppercase tracking-widest w-full sm:w-auto">
                              <CheckCircle className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> Deployed Matrix
                           </div>
                        ) : (
                           <Button onClick={() => handleSetActiveFlow(flow._id)} disabled={isActionLoading(flow._id, 'set-active')} className="bg-indigo-600 hover:bg-indigo-700 h-8 sm:h-8 px-4 rounded-lg font-black text-[12px] sm:text-[14px] uppercase tracking-widest shadow-md w-full sm:w-auto">
        ...
        <TabsContent value="templates" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 pt-2">
           {templateFlows.length === 0 ? (
             <div className="col-span-full py-16 sm:py-20 text-center bg-white rounded-2xl border border-slate-100 border-dashed">
                ...
                <p className="text-[14px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest">Library is currently empty</p>
             </div>
           ) : (
             templateFlows.map(template => (
               <Card key={template._id} className="rounded-xl border-slate-100 shadow-sm overflow-hidden hover:shadow-md transition-all flex flex-col">
                  ...
                  <CardHeader className="py-4 px-4 sm:px-6">
                     <CardTitle className="text-[15px] sm:text-sm font-bold text-slate-800 uppercase tracking-tight truncate">{template.name || template.flowName}</CardTitle>
                     <p className="text-[15px] sm:text-[14px] text-slate-500 line-clamp-2 mt-1 min-h-[2.5rem]">{template.description || "Reusable blueprint for organizational workflows"}</p>
                  </CardHeader>
                  <CardContent className="pb-4 px-4 sm:px-6 space-y-4 mt-auto">
                     <div className="flex items-center gap-4 text-[15px] sm:text-[14px] font-bold text-slate-400 uppercase tracking-widest">
                     ...
                     <Button onClick={() => handleDuplicateFlow(template._id)} className="w-full bg-slate-900 hover:bg-black text-white h-9 rounded-lg font-black text-[15px] sm:text-[14px] uppercase tracking-widest">
