...
export const importAPI = {
  ...
  downloadTemplate: async (type: 'companies' | 'departments' | 'users' | 'drilldown-hierarchy'): Promise<void> => {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api'||'https://chatbot-portal-latest.vercel.app/api'||'https://connect-pugarch.vercel.app/api'}/import/template/${type}`, {
      headers: {
